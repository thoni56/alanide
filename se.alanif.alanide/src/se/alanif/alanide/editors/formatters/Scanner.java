...
public class Scanner {
	...
	static {
		...
		for (int i = 224; i <= 246; ++i) start.set(i, 1);
		for (int i = 248; i <= 254; ++i) start.set(i, 1);
		...
		literals.put("opaque", new Integer(4));
		literals.put("container", new Integer(5));
		...
		loop: for (;;) {
			switch (state) {
				...
				case 1:
					...
					if (ch == '$' || ch >= '0' && ch <= '9' || ch == '_' || ch >= 'a' && ch <= 'z' || ch >= 224 && ch <= 246 || ch >= 248 && ch <= 254) {AddCh(); state = 1; break;}

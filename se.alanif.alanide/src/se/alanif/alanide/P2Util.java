...
public class P2Util {
	...
	static IStatus checkForUpdates(IProvisioningAgent agent, IProgressMonitor monitor) throws OperationCanceledException {
		...
		if (status.getSeverity() == IStatus.OK) {
			// Update as suggested by the operation.
			ProvisioningJob job = operation.getProvisioningJob(null);
			// If you get a null pointer exception here when running inside Eclipse,
			// you need to update your run configuration and set a check in the box:
